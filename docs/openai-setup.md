...
Первое фото объявления по умолчанию уходит в анализ (`AI_VISION_ENABLED=true`):
по картинке модель отличает настоящий велосипед от аксессуара — детского кресла,
велотуфель, спиннинг-тренажёра — и опознаёт тип, когда текст пустой. Работает на
основной модели; чтобы отдать зрение другой, задайте `OPENAI_VISION_MODEL`.

Пока слой в тени (`AI_SHADOW_MODE=true`), классификация ни на что не влияет.
Чтобы AI начал **убирать** очевидные не-велосипеды из Telegram, после проверки
shadow-данных снимите тень и допустите слой к статусам:

```
AI_SHADOW_MODE=false
AI_CAN_AFFECT_DEAL_STATUS=true
```

Это одностороннее право: AI может только скрыть карточку, которую уверенно счёл
не-велосипедом (порог `AI_MIN_REJECT_CONFIDENCE`, по умолчанию 0.75), но не
поднять статус и не тронуть цену. До снятия тени очевидный мусор всё равно
отсекает дешёвый фильтр по словам в заголовке — вызова AI на него не тратится.
...
Prompt version: listing-analysis-v1.2.0

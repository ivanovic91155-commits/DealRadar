...
class TelegramClient:
    ...
    def send_listing(self, listing: Listing, retail_enabled: bool, verdict=None) -> int:
        if not self.chat_id:
            raise ValueError("TELEGRAM_CHAT_ID is not configured")
        ...
        header = f"🚲 <b>Новое на {html.escape(marketplace)}</b>"
        score_block = ""
        if verdict is not None:
            tier_badge = {"hot": "🔥 <b>ГОРЯЧО</b>", "notify": "📩 сделка", "archive": "🔕"}
            badge = tier_badge.get(verdict.tier, "")
            header = f"{badge} · DealScore <b>{verdict.score}</b>/100"
            if verdict.tier == "hot":
                header = f"🔥 <b>ГОРЯЧО · успей первым</b>\nDealScore <b>{verdict.score}</b>/100"
            lines_extra = []
            if verdict.expected_profit_czk is not None and verdict.expected_profit_czk > 0:
                lines_extra.append(f"📈 Ожид. прибыль: <b>~{format_czk(verdict.expected_profit_czk)}</b>")
            if verdict.reasons:
                lines_extra.append("✓ " + "; ".join(html.escape(r) for r in verdict.reasons[:3]))
            if verdict.red_flags:
                lines_extra.append("⚠️ " + "; ".join(html.escape(f) for f in verdict.red_flags[:3]))
            score_block = "\n".join(lines_extra)

        ai_line = "🔎 Ищу цены нового велосипеда…" if retail_enabled else ""
        text = (
            f"{header}\n\n"
            f"<b>{title}</b>\n"

...
def _permissive_scoring() -> ScoringConfig:
    """Скоринг с порогом notify=0: bootstrap-тесты проверяют маршрутизацию по
    ID, а не качество сделки, поэтому пропускаем всё оценённое в notify."""
    cfg = ScoringConfig()
    cfg.routing.notify_min_score = 1.0
    cfg.validate()
    return cfg
